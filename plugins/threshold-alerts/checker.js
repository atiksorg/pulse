...
async function checkAlertConfig(db, cfg, opts = {}) {
  ...
  try {
    ...
    db.prepare(
      `INSERT INTO alert_history (config_id, dashboard_id, src, ts, value, direction, status, error_message, trigger_type)
       VALUES (?, ?, ?, ?, NULL, NULL, 'error', ?, ?)`
    ).run(cfg.id, cfg.dashboard_id, cfg.src, now, e.message.slice(0, 300), triggerType);
...
function logHistory(db, cfg, ts, value, direction, telegramResult, triggerType) {
  ...
  db.prepare(
    `INSERT INTO alert_history (config_id, dashboard_id, src, ts, value, direction, status, error_message, trigger_type)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(cfg.id, cfg.dashboard_id, cfg.src, ts, value, direction, status, errorMessage, triggerType);
}
